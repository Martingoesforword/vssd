...
namespace sjh {
	namespace tool { 
		...
		void string::Split(std::wstring str, std::vector<std::wstring> &ret, std::wstring pattern)
		{
			if (pattern.empty())
			{
				pattern = L" ";
			...
			while (index != str.npos && math::IsOuShu( string::SameTimes(str, L"\"", start, index)))
			{
				if (start != index)
		...
		int string::SameTimes(std::wstring str, std::wstring pattern, int Start, int End)
		{
			int times = 0;
		...
		void string::Trim(std::wstring & s)
		{
			if (!s.empty())
			{
		...
		size_t string::PushLengthValue(size_t Uint4, std::vector<wchar_t> &Byte)
		{
		...
		void string::PushString(std::wstring Str, std::vector<wchar_t> &Byte)
		{
		...
		void string::PushWcharVector(std::vector<wchar_t> &StringArray, std::vector<wchar_t> &Byte)
		{
		...
		unsigned int string::GetLengthValue(const std::vector<wchar_t>& ByteVssd, int &Pos)
		{
		...
		std::wstring string::GetString(const std::vector<wchar_t>& ByteVssd, int &Pos)
		{
		...
		void string::GetWcharVector(std::vector<wchar_t> &WcharVector, std::vector<wchar_t> &ByteVssd, int &Pos)
		{
			unsigned int LengthValue = GetLengthValue(ByteVssd, Pos);
			for (size_t i = 0; i < LengthValue; i++)
			{
		...
		void string::PushToRealFile(std::wstring JumpTo, std::vector<wchar_t> aSerial)
		{
		...
		void string::GetFromRealfile(std::wstring GetFrom, std::vector<wchar_t> aSerial)
		{
			...
			else
			{
				...
				int pos = 0;
				Bytelength = string::GetLengthValue(aSerial, pos);
				aSerial.clear();
		...
		std::wstring string::GetTimeWString(time_t tm)
		{
		...
		std::wstring string::GetSizeWString(size_t Size)
		{
			std::wstringstream oss;
		...
		std::string string::GetTimeString(time_t tm)
		{
			return string::WStringToString(GetTimeWString(tm));
		}

		std::string string::GetSizeString(size_t Size)
		{
			return string::WStringToString(GetSizeWString(Size));
		}

		void string::CheckQuotation(std::wstring& Str)
		{
			if (Str[0] == '\"') Str = Str.substr(1, Str.size() - 2); 
		}


		std::string string::WStringToString(const std::wstring &wstrSrc)
		{
		...
		std::wstring string::StringToWString(const std::string &strSrc)
		{
		...
		wchar_t string::WStringMatch(std::wstring str1, std::wstring str2)
		{
			int slen1 = str1.size();
			int slen2 = str2.size();
