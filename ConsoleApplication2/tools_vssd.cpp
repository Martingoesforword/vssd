...
namespace sjh { 
	...
	void tool::stringtools::Trim(std::wstring & s)
	{
		if (!s.empty())
		{
	...
	void tool::stringtools::PushString(std::wstring Str, std::vector<wchar_t> &Byte)
	{
	...
	void tool::stringtools::PushWcharVector(std::vector<wchar_t> &StringArray, std::vector<wchar_t> &Byte)
	{
	...
	unsigned int tool::stringtools::GetLengthValue(const std::vector<wchar_t>& ByteVssd, int &Pos)
	{
	...
	std::wstring tool::stringtools::GetString(const std::vector<wchar_t>& ByteVssd, int &Pos)
	{
	...
	void tool::stringtools::GetWcharVector(std::vector<wchar_t> &WcharVector, std::vector<wchar_t> &ByteVssd, int &Pos)
	{
		unsigned int LengthValue = GetLengthValue(ByteVssd, Pos);
		for (size_t i = 0; i < LengthValue; i++)
		{
	...
	void tool::stringtools::PushToRealFile(std::wstring JumpTo, std::vector<wchar_t> aSerial)
	{
	...
	void tool::stringtools::GetFromRealfile(std::wstring GetFrom, std::vector<wchar_t> aSerial)
	{
		...
		else
		{
			...
			int pos = 0;
			Bytelength = tool::stringtools::GetLengthValue(aSerial, pos);
			aSerial.clear();
	...
	std::wstring tool::stringtools::GetTimeWString(time_t tm)
	{
	...
	std::string tool::stringtools::GetTimeString(time_t tm)
	{
	...
	std::string tool::stringtools::WStringToString(const std::wstring &wstrSrc)
	{
	...
	std::wstring tool::stringtools::StringToWString(const std::string &strSrc)
	{
	...
	wchar_t tool::stringtools::WStringMatch(std::wstring str1, std::wstring str2)
	{
		int slen1 = str1.size();
		int slen2 = str2.size();
